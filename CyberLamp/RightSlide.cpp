...
class RightSlide : public Transition {
	double previousOffset;

  public:
	RightSlide(Mode *from, Mode *to, uint16_t duration, EasingBase *smoother)
	...
	void refresh() {
		...
		// White stripe
		for (uint16_t y = 0; y < HEIGHT; ++y) {
			pixels[y][WIDTH - 1 - (uint16_t)offset].setColorCode(CRGB::White);
		}
...
#endif
				pixels[y][WIDTH - 1 - (uint16_t)offset + dist].addToRGB(shadeValue);
			}
		}
